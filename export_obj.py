...
def write_file(filepath, objects, actions, scene,
               EXPORT_TRI=False,
               EXPORT_EDGES=False,
                    ...
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        for modifier in ob.modifiers:
                            if modifier.type== "ARMATURE":
                                ...
                                fw("useskeleton "+ armature.name+ "\n")


                                skeleton_library_file.write("skeleton "+ armature.name+ "\n")

                                for bone in armature.data.bones:
                                    skeleton_library_file.write("bone "+ bone.name+ "\n")

                                    if bone.parent is not None:
                                        skeleton_library_file.write("parent "+ bone.parent.name+ "\n")
                                
                                    parent_matrix= None
                                    parent= bone.parent
                                    while parent is not None:
                                        parent_matrix= parent.matrix* parent_matrix if parent_matrix is not None else parent.matrix
                                        parent= parent.parent
                                    if parent_matrix is None:
                                        parent_matrix= Matrix()

                                    tail= EXPORT_GLOBAL_MATRIX* bone.tail_local
                                    head= EXPORT_GLOBAL_MATRIX* bone.head_local
                                    forward= EXPORT_GLOBAL_MATRIX* (parent_matrix* bone.y_axis);
                                    up= EXPORT_GLOBAL_MATRIX* (parent_matrix* bone.z_axis);
                                    right= EXPORT_GLOBAL_MATRIX* (parent_matrix* bone.x_axis);

                                    skeleton_library_file.write("tail "+ str(tail.x)+ " "+ str(tail.y)+ " "+ str(tail.z)+ " "+ "\n")
                                    skeleton_library_file.write("head "+ str(head.x)+ " "+ str(head.y)+ " "+ str(head.z)+ " "+ "\n")
                                    skeleton_library_file.write("forward "+ str(forward.x)+ " "+ str(forward.y)+ " "+ str(forward.z)+ " "+ "\n")
                                    skeleton_library_file.write("up "+ str(up.x)+ " "+ str(up.y)+ " "+ str(up.z)+ " "+ "\n")
                                    skeleton_library_file.write("right "+ str(right.x)+ " "+ str(right.y)+ " "+ str(right.z)+ " "+ "\n")

                                
                                if armature.animation_data is not None:
                                    for nla_track in armature.animation_data.nla_tracks:
                                        skeleton_library_file.write("action "+ nla_track.name+ "\n")
                                        for strip in nla_track.strips:
                                            action= strip.action
                                #for action in actions:                       
                                #    skeleton_library_file.write("action "+ action.name+ "\n")
                                            frame_count= action.frame_range[1]- action.frame_range[0]
                                            for frame_index in range(int(frame_count+ 1)):
                                                frame= int(action.frame_range[0])+ frame_index
                                                skeleton_library_file.write("frame "+ str(frame)+ "\n")
                                                ...
                                                for group in action.groups:
                                                    ...
                                                    for channel in group.channels:
                                                        type_= channel.data_path.split(".")[-1]
                                                        print(type_)
                                                        index= channel.array_index

                                                        if type_== "location":
                                                            position[index]= channel.evaluate(frame)
                                                        elif type_== "scale":
                                                            scale[index]= channel.evaluate(frame)
                                                        elif not type_.split("_")[1]== "quaternion":
                                                            rotation[index]= channel.evaluate(frame)
                                                            
                                                    bone_name= group.name.replace(".FK", "")
                                                    skeleton_library_file.write("key "+ bone_name+ " position "+ str(position[0])+ " "+ str(position[1])+ " "+ str(position[2])+ " "+ "\n")
                                                    skeleton_library_file.write("key "+ bone_name+ " rotation "+ str(rotation[0])+ " "+ str(rotation[2])+ " "+ str(rotation[1])+ " "+ "\n")
                                                    skeleton_library_file.write("key "+ bone_name+ " scale "+ str(scale[0])+ " "+ str(scale[1])+ " "+ str(scale[2])+ " "+ "\n")
...
def _write(context, filepath,
    ...
    with ProgressReport(context.window_manager) as progress:
        ...
        for frame in scene_frames:
            ...
            write_file(full_path, objects, actions, scene,
                       EXPORT_TRI,
                       EXPORT_EDGES,
                       EXPORT_SMOOTH_GROUPS,
                       EXPORT_SMOOTH_GROUPS_BITFLAGS,
