...
uniform float indirect_buffer_scale;
uniform vec2 indirect_buffer_offset;
...
{ 
  ...
  color= vec4(vec3(1)* 0.0+ diffuse* 1.0+ specular* 1.0+ 0.0* texture(indirect_buffer, (normalized_coordinates)* indirect_buffer_scale+ indirect_buffer_offset).xyz, 1);
